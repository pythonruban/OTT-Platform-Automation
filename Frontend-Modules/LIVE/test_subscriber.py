...
xpaths = {
    ...
    "subscriber_button": "//button[@id='subscriber-now-button']",
...
class TestsubscriberSignupAutomation:

    def test_subscriber_signup_complete_flow(self, browser_setup):
        """Complete subscriber Signup Automation Flow - Single Test Function"""
        ...
        def handle_subscriber_and_signup():
            """Handle subscriber detection and complete signup process"""
            nonlocal signup_completed
            try:
                wait_obj = WebDriverWait(driver, 10)
                
                # Check if subscriber button is present
                try:
                    subscriber_button = wait_obj.until(EC.presence_of_element_located((By.XPATH, xpaths["subscriber_button"])))
                    print("🔒 subscriber content detected - starting signup process")
                    allure.attach(driver.get_screenshot_as_png(), name="subscriber_Detected", attachment_type=AttachmentType.PNG)
                    
                    # Click subscriber button
                    if not smart_wait_and_click(By.XPATH, xpaths["subscriber_button"], "subscriber Subscribe Button"):
                        return False
                    ...
                    print("ℹ️ No subscriber popup detected - content is free")
                    return True
                    
            except Exception as e:
                print(f"❌ Error in subscriber/signup process: {e}")
                allure.attach(driver.get_screenshot_as_png(), name="subscriber_Signup_Error", attachment_type=AttachmentType.PNG)
                return False
        ...
        try:
            ...
            with allure.step("Step 3: Process Categories and Videos for subscriber"):
                        ...
                        try:
                            ...
                            for video_index in range(video_count):
                                ...
                                try:
                                    ...
                                    # Handle subscriber and signup
                                    subscriber_result = handle_subscriber_and_signup()
                                    
                                    if signup_completed:
                                        print(f"🎉 Signup completed successfully on video {video_index+1}!")
                                        break
                                    elif subscriber_result:
                                        # No subscriber, go back and try next video
                                        driver.back()
            ...
            # Final results
            if signup_completed:
                print("🎉 subscriber Signup automation completed successfully!")
                allure.attach(driver.get_screenshot_as_png(), name="Final_Success", attachment_type=AttachmentType.PNG)
                assert True
            else:
                print("ℹ️ No subscriber content found requiring signup")
                allure.attach(driver.get_screenshot_as_png(), name="No_subscriber_Found", attachment_type=AttachmentType.PNG)
                assert True  # Still pass if no subscriber content was found
